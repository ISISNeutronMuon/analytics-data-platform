...
def test_create_partition_spec_when_hint_none_or_empty(sample_schema):
    """Test that None partition hint returns unpartitioned spec."""
    assert create_partition_spec(None, sample_schema) == UNPARTITIONED_PARTITION_SPEC
    assert create_partition_spec({}, sample_schema) == UNPARTITIONED_PARTITION_SPEC


def test_create_partition_spec_single_column_identity(sample_schema):
...
def test_create_partition_spec_single_column_date_transforms(
    sample_schema, transform, expected_name, expected_transform_type
):
...
def test_create_partition_spec_single_column_hour(sample_schema):
...
def test_create_partition_spec_truncate_transform(sample_schema):
...
def test_create_partition_spec_bucket_transform(sample_schema):
...
def test_create_partition_spec_multiple_columns(sample_schema):
...
def test_create_partition_spec_multiple_columns_field_id_sequence(sample_schema):
...
def test_create_partition_spec_nonexistent_column_raises_error(sample_schema):
