...
from elt_common.typing import ELTJobManifest, TableProperties
...
def test_run_ingest_with_simple_extract_class(elt_job: ELTJobManifest, mock_iceberg_io: MagicMock):
    ...
    for index, call in enumerate(call_args_list):
        ...
        assert call_args[1] == TableProperties(
...
def test_run_ingest_with_write_mode_replace_first_replaces_then_appends(
    ...
    assert first_call_args[1] == TableProperties(write_mode="replace")
    ...
    assert second_call_args[1] == TableProperties(write_mode="replace")
